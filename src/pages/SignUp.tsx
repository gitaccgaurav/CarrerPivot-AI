...
export default function SignUp() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!sent) { const { error: e } = await supabase.auth.signInWithOtp({ email, options: { shouldCreateUser: true, emailRedirectTo: `${window.location.origin}/` } }); setLoading(false); if (e) setError(e.message); else setSent(true); return; }
