  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!sent) {
      ...
      if (remainingSeconds > 0) {
        ...
        setError(`Please wait ${formatCooldown(remainingSeconds)} before requesting another code.`);
        setLoading(false);
        return;
      }
      ...
      setLoading(false);
      if (otpError) {
        if (isAuthEmailRateLimit(otpError.message)) {
          startAuthEmailRateLimitCooldown(normalizedEmail);
          setCooldownSeconds(getAuthEmailCooldownSeconds(normalizedEmail));
          setError('Supabase has temporarily blocked new email codes. Wait a few minutes, then try again.');
        } else {
          setError(otpError.message);
        }
      } else {
        startAuthEmailCooldown(normalizedEmail);
        setCooldownSeconds(getAuthEmailCooldownSeconds(normalizedEmail));
