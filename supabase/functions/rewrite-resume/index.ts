...
async function callClaude(messages: { role: string; content: string }[]) {
  ...
  if (!apiKey) throw new Error('Server misconfiguration: missing ANTHROPIC_API_KEY');
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. Call Claude, with one retry if JSON parsing fails
    ...
    try {
      ...
      const raw = await callClaude([
        ...
        { role: 'user', content: 'Return valid JSON only, starting with the opening brace. No markdown, no commentary.' },
      ]);
      parsed = tryParseJson('{' + raw);
    }

    // 3. Save to rewrites table
